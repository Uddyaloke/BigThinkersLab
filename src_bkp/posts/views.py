...
def post_detail(request, slug=None):	
	...
	relevant_postgroups = Postgroup.objects.filter(posts=instance.id)
	...
	context = {
		...
		"instance": instance,
		"relevant_postgroups": relevant_postgroups,
...
def post_list(request):
	...
	queryset_list = Post.published_posts.active()
...
def post_tag_list(request, slug=None):
	today = timezone.now().date()
	queryset_list = Post.published_posts.active()
	if request.user.is_staff or request.user.is_superuser:
		tag_id = Post.tags.filter(slug=slug).values_list('post', flat=True)
		queryset_list = Post.objects.filter(id__in=tag_id)
...
class PostDetailJSONView(PostMixinDetailView, DetailView):
    template_name = 'post/post_ajax.html'
